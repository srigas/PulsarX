...
DOUBLE_PRECISION = [True]         # True → 64-bit, False → 32-bit
BETA_SEP         = [0.025]        # separatrix update step size β
TOL              = [(15e-4, 1e-1)] # (tol_sep, tol_dp) convergence pair
CYCLE_CONFIG     = [{0: 20000, 6: 10000}]  # training schedule per cycle
SYM_SEP          = [True]         # True → SymmetricSeparatrix
...
- **`logs/<experiment>/log.txt`** — full stdout captured during training, including loss values, LRA and resampling events, pressure diagnostics and convergence notifications.
- **`model_ckpt/<experiment>/`** — Orbax checkpoints for the closed-line-region network (`model_c`), open-line-region network (`model_o`), and separatrix network (`model_sep`). These can be reloaded with `src/io.py::load_pinn_models`.

The experiment name is derived from all configurable parameters, so no output from one run overwrites another.
